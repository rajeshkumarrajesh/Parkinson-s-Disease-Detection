...
# Enhanced Peafowl Optimization Algorithm (EPOA) Starting Line No. 30
def Proposed(population, fobj, VRmin, VRmax, max_iter):
    ...
    for t in range(max_iter):
        for i in range(population_size):
            worstfit = max(personal_best_fitness)
            r = ((global_best_fitness ^ 2) / (worstfit ^ 2))
            # Update velocity
            velocities[i] = velocities[i] + 0.1 * r * (
                    personal_best_positions[i] - population[i]) + 0.1 * r * (
